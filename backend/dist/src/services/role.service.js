...
export const createRoleService = async (roleName, description) => {
    ...
    if (existedRole)
        throw new AppError(`Hiện tại role ${roleUpperCase} đã tồn tại`, 409);
...
export const deleteRoleService = async (roleName) => {
    //Chuẩn hóa roleName trước khi xử lý
    const roleUpperCase = roleName.toUpperCase();
    //tạo biến existedRole để xem roleName đã tồn tại chưa
    const existedRole = await findByRoleNameByRoleName(roleUpperCase);
    if (!existedRole)
        throw new AppError(`Không tìm thấy ${roleUpperCase} để xóa`, 404);
