...
function buildCompletedOrderWhere(from, to) {
    return {
...
function buildStatusDateWhere(status, dateField, from, to) {
    return {
        status,
        OR: [
            {
...
function buildStatisticsBuckets(from, to, preset) {
    const diffDays = Math.max(Math.ceil((to.getTime() - from.getTime()) / (24 * 60 * 60 * 1000)), 1);
    const unit = preset === "today" || diffDays <= 1
        ? "hour"
...
async function getLowStockItems(lowStockThreshold, page, limit) {
...
async function getProductStats(where, take = 5) {
...
export async function getDashboardSummaryAnalyticsService(query) {
    ...
    const [revenueAggregate, ordersToday, completedOrdersToday, deliveryFailedToday, pendingOrders, cancelledOrders, cancelledOrdersToday, warrantyRequestsToday, newCustomersToday, newCustomersLast7Days, newCustomersThisMonth, revenueOrders, productStats, lowStockResult,] = await Promise.all([
...
export async function getAdminStatisticsService(query) {
    ...
    const [revenueAggregate, totalOrders, completedOrders, cancelledOrders, deliveryFailedOrders, pendingOrders, revenueOrders, productStats, newCustomers, totalCustomers, warrantyRequests, topWarrantyProductsRaw,] = await Promise.all([
        prisma.orders.aggregate({
    ...
    const warrantyProductMap = new Map();
