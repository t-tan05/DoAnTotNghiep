...
export const createBrandService = async (brandName, description) => {
    ...
    if (existedBrand)
        throw new AppError("Thương hiệu sản phẩm đã tồn tại", 409);
...
export const updateBrandService = async (brandId, brandName, description) => {
    const brand = await findBrandById(brandId);
    if (!brand)
        throw new AppError("Không tìm thấy thương hiệu sản phẩm", 404);
    const displayName = brandName.trim();
    ...
    if (existedBrand && existedBrand.brand_id !== brandId)
        throw new AppError("Tên thương hiệu sản phẩm đã tồn tại", 409);
