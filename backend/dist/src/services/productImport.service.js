...
const buildComboKey = (attributeValueIds) => {
    return [...attributeValueIds].sort().join("|");
};
const readRowsFromExcel = (file) => {
    ...
    const duplicateAttributeHeaders = new Set();
    const normalizedHeaderSet = new Set();
    for (const header of headers) {
        if (BASE_HEADERS.includes(header))
            continue;
    ...
    if (duplicateAttributeHeaders.size > 0) {
        throw new AppError(`File Excel có cột thuộc tính bị trùng: ${[...duplicateAttributeHeaders].join(", ")}`, 400);
    }
    const attributeColumns = headers.filter((header) => !BASE_HEADERS.includes(header));
    return jsonRows.map((row, index) => {
        const attributeValuesByColumn = {};
...
const getAttributeValueIds = (row, attributeMap) => {
...
export const importProductsFromExcelService = async (file, createdBy) => {
    if (!file) {
        throw new AppError("Vui lòng chọn file Excel.", 400);
    }
    const rows = readRowsFromExcel(file);
    const errors = [];
    const skuInFileSet = new Set();
    ...
    const [existedProducts, existedSkus, brands, categories, attributes,] = await Promise.all([
