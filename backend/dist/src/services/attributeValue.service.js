import { createAttributeValues, deleteAttributeValue, findAttributeValueByAttributeAndNormalizedValue, findAttributeValueById, findAttributeValuesByNormalizedValues, updateAttributeValue } from "#models/attributeValue.model";
...
export const createAttributeValueService = async (data) => {
    ...
    for (const attributeId of attributeIds) {
        ...
        if (!attribute)
            throw new AppError(`Thuộc tính ${attributeId} không tồn tại`, 404);
    }
    for (const attributeId of attributeIds) {
...
export const updateAttributeValueService = async (attributeValueId, payload) => {
    ...
    if (payload.displayOrder !== undefined)
        attributeValueData.display_order = payload.displayOrder;
