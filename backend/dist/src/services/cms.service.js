...
const getMetadataArray = (metadata, key) => {
...
const addInFilter = (target, field, values) => {
    if (values.length === 1) {
...
const buildCmsProductScopeWhere = (collection) => {
...
const mergeVariantWhere = (...whereList) => {
...
const buildCmsQueryFilterWhere = (query) => {
    const where = {};
    const productWhere = {};
    if (query.categoryId) {
...
export const getPublicCmsCollectionService = async (slug) => {
...
export const getPublicCmsCollectionProductsService = async (slug, query) => {
    const collection = await findActiveCmsCollectionBySlug(slug);
    if (!collection) {
        throw new AppError("Không tìm thấy trang CMS.", 404);
    }
    ...
    const where = mergeVariantWhere(buildCmsProductScopeWhere(collection), buildCmsQueryFilterWhere(query));
    ...
    const groupedProducts = sortPublicProductCards(groupPublicVariants(items), query.sortBy);
...
export const getPublicCmsCollectionFiltersService = async (slug) => {
    const collection = await findActiveCmsCollectionBySlug(slug);
    if (!collection) {
        throw new AppError("Không tìm thấy trang CMS.", 404);
    }
...
export const getAdminCmsCollectionsService = async (query) => {
    const page = Number(query.page) > 0 ? Number(query.page) : 1;
...
export const createCmsCollectionService = async (data) => {
...
export const updateCmsCollectionService = async (collectionId, data) => {
    const collection = await findCmsCollectionById(collectionId);
    if (!collection) {
        throw new AppError("Không tìm thấy CMS collection.", 404);
    }
    ...
    if (data.pageType !== undefined)
        updateData.page_type = data.pageType;
...
export const deleteCmsCollectionService = async (collectionId) => {
    const collection = await findCmsCollectionById(collectionId);
    if (!collection) {
        throw new AppError("Không tìm thấy CMS collection.", 404);
    }
...
export const createCmsSectionService = async (collectionId, data) => {
    const collection = await findCmsCollectionById(collectionId);
    if (!collection) {
        throw new AppError("Không tìm thấy CMS collection.", 404);
    }
...
export const updateCmsSectionService = async (sectionId, data) => {
    ...
    if (data.sectionType !== undefined)
        updateData.section_type = data.sectionType;
    if (data.title !== undefined)
        updateData.title = data.title;
    if (data.subtitle !== undefined)
        updateData.subtitle = data.subtitle;
    if (data.backgroundImage !== undefined)
        updateData.background_image = data.backgroundImage;
...
export const deleteCmsSectionService = async (sectionId) => {
    const section = await findCmsSectionById(sectionId);
    if (!section) {
        throw new AppError("Không tìm thấy CMS section.", 404);
    }
...
export const createCmsSectionItemService = async (sectionId, data) => {
    const section = await findCmsSectionById(sectionId);
    if (!section) {
        throw new AppError("Không tìm thấy CMS section.", 404);
    }
...
export const updateCmsSectionItemService = async (itemId, data) => {
    ...
    if (data.variantId !== undefined)
        updateData.variant_id = data.variantId;
    if (data.blogId !== undefined)
        updateData.blog_id = data.blogId;
    if (data.metadata !== undefined)
        updateData.metadata = data.metadata;
    if (data.sortOrder !== undefined)
        updateData.sort_order = data.sortOrder;
...
export const deleteCmsSectionItemService = async (itemId) => {
    const item = await findCmsSectionItemById(itemId);
    if (!item) {
        throw new AppError("Không tìm thấy CMS section item.", 404);
    }
...
export const createCmsRuleService = async (collectionId, data) => {
    const collection = await findCmsCollectionById(collectionId);
    if (!collection) {
        throw new AppError("Không tìm thấy CMS collection.", 404);
    }
...
export const updateCmsRuleService = async (ruleId, data) => {
    ...
    if (data.attributeName !== undefined)
        updateData.attribute_name = data.attributeName;
    if (data.attributeValue !== undefined)
        updateData.attribute_value = data.attributeValue;
    if (data.keyword !== undefined)
        updateData.keyword = data.keyword;
    if (data.minPrice !== undefined)
        updateData.min_price = data.minPrice;
    if (data.maxPrice !== undefined)
        updateData.max_price = data.maxPrice;
    if (data.sortBy !== undefined)
        updateData.sort_by = data.sortBy;
...
export const deleteCmsRuleService = async (ruleId) => {
    const rule = await findCmsRuleById(ruleId);
    if (!rule) {
        throw new AppError("Không tìm thấy CMS rule.", 404);
    }
