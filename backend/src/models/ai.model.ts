...
function extractMeaningfulWords(text: string) {
...
function detectProductIntent(text: string) {
    const normalized = normalizeSearchText(text);
...
function buildIntentWhere(intentTerms: string[]) {
    if(!intentTerms.length) return null;
...
function scoreVariantForAi(variant: any, text: string, budget: number | null, intentTerms: string[]) {
    ...
    const haystack = normalizeSearchText([
        ...
        ...variant.product_variant_specs.map((spec: any) => `${spec.spec_key} ${spec.spec_value}`),
    ].filter(Boolean).join(" "));
    ...
    if(intentTerms.some((term) => haystack.includes(term))) score += 1000;

    words.forEach((word) => {
        if(haystack.includes(word)) score += 40;
    });

    if(budget && price > 0) {
        const distance = Math.abs(price - budget) / budget;
        score += Math.max(0, 300 - distance * 300);
        if(price <= budget) score += 80;
        if(price > budget * 1.1) score -= 500;
    }
...
export async function findProductsForAi(keyword: string) {
    ...
    if(intentWhere) andConditions.push(intentWhere);
...
export function attachGuestConversationsToUser(guestId: string, userId: string) {
