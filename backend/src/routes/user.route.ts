...
import { createEmployeeController, delUserController, getAllUsersController, lockUserController, profileController, unlockUserController, updatePasswordController, updateProfileController } from "#controllers/user.controller";
import { Validate } from "#middlewares/Validate";
import { createEmployeeSchema, updatePasswordSchema } from "#validations/user.validation";
import { changePasswordLimiter } from "#config/rateLimit";

const router = Router();

router.get("/", VerifyToken, CheckRole("ADMIN"), getAllUsersController);
router.post("/employees", VerifyToken, CheckRole("ADMIN"), Validate(createEmployeeSchema), createEmployeeController);
