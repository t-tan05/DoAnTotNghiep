...
import {
    ...
    findProductLineById,
    findProductLineByScopeSlug,
    getProductLinesWithQuery,
    updateProductLine,
} from "#models/productLine.model";
import {
    CreateProductLinePayload,
    ProductLineListQuery,
    UpdateProductLinePayload,
} from "#types/productLine.type";
import AppError from "#utils/AppError";
import { createSlug } from "#utils/createSlug";
import { normalizeText } from "#utils/normalizeText";
import crypto from "crypto";

const buildLineSlug = (lineName: string) => createSlug(lineName.trim());

const ensureBrandAndCategory = async(brandId: string, categoryId: string) => {
...
const ensureUniqueProductLine = async({
    lineId,
    brandId,
    categoryId,
    slug,
...
export const createProductLineService = async(data: CreateProductLinePayload) => {
...
export const getAllProductLinesService = async(params: ProductLineListQuery) => {
...
export const getProductLineByIdService = async(lineId: string) => {
...
export const updateProductLineService = async(
    lineId: string,
    data: UpdateProductLinePayload,
) => {
    const productLine = await findProductLineById(lineId);
...
export const deleteProductLineService = async(lineId: string) => {
    const productLine = await findProductLineById(lineId);
