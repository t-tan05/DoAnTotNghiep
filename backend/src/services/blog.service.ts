...
import type { BlogListQuery, CreateBlogPayload, UpdateBlogPayload } from "#types/blog.type";
...
const buildUniqueSlug = async(title: string, excludeProstId?: string) => {
...
export const getPublicBlogsService = async (params: BlogListQuery) => {
...
export const getPublicBlogDetailService = async (postId: string) => {
...
export const getAdminBlogsService = async (params: BlogListQuery) => {
...
export const getAdminBlogDetailService = async (postId: string) => {
    const blog = await findBlogById(postId);

    if (!blog) throw new AppError(BLOG_NOT_FOUND_MESSAGE, 404);
