...
import type { CreateProductAttributePayload, UpdateProductAttributePayload } from "#types/productAttribute.type";

export const createProductAttributeService = async(data: CreateProductAttributePayload) => {
...
export const updateProductAttributeService = async(attributeId: string, payload: UpdateProductAttributePayload) => {
    const productAttribute = await findProductAttributeById(attributeId);

    if(!productAttribute) throw new AppError("Không tìm thấy mã thuộc tính sản phẩm", 404);

    const productAttributeData: Prisma.product_attributesUpdateInput = {};
    ...
    if(payload.displayOrder !== undefined) productAttributeData.display_order = payload.displayOrder;
