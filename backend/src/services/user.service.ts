...
export const lockUserService = async(userId: string, reason?: string) => {
    const existedUser = await findUserById(userId);

    if(!existedUser) throw new AppError("Người dùng không tồn tại", 404);

    if(existedUser.status === "LOCKED") throw new AppError("Tài khoản này đã bị khóa trước đó", 409);
...
export const unlockUserService = async(userId: string) => {
    const existedUser = await findUserById(userId);

    if(!existedUser) throw new AppError("Người dùng không tồn tại", 404);

    if(existedUser.status === "ACTIVE") throw new AppError("Tài khoản này chưa bị khóa", 409);
...
export const updatePasswordService = async(userId: string, currentPassword: string, newPassword: string, confirmPassword: string) => {
    ...
    if(existedPassword) throw new AppError("Mật khẩu mới không được trùng với mật khẩu cũ.", 409);
