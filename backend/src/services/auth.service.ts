...
export const loginService = async (email: string, password: string) => {
    ...
    if (!user) throw new AppError("Email hoặc mật khẩu không đúng", 404);
    ...
    if(!user.verified) throw new AppError("Vui lòng xác thực tài khoản trước khi đăng nhập", 403);
...
export const verifyEmailService = async(email: string, verifyToken: string) => {
    const user = await findUserByEmail(email);
    ...
    if(user.verified) throw new AppError("Tài khoản đã được xác thực", 409);

    //Kiểm tra mã xác thực và thời gian có hợp lệ không
    if(!user.verify_token || !user.verify_token_expire) throw new AppError("Mã xác thực không tồn tại", 400);

    //So sánh mã xác thực có đúng chưa
    if(user.verify_token !== hashVerifyToken) throw new AppError("Mã xác thực không đúng", 400);

    //Kiểm tra thời gian của mã xác thực hợp lệ không
    if(user.verify_token_expire < new Date()) throw new AppError("Mã xác thực đã hết hạn", 400);
...
export const resendVerifyEmailService = async(email: string) => {
    ...
    if(user.verified) throw new AppError("Tài khoản này đã được xác thực", 409);
    ...
    if(user.verify_token_expire){
        ...
        if(Date.now() < canResendAt) throw new AppError("Vui lòng chờ 60 giây trước khi gửi lại mã", 429);
    }
...
export const verifyResetCodeService = async(email: string, resetCode: string) => {
    const user = await findUserByEmail(email);
    ...
    if(!user.reset_token || !user.reset_token_expire) throw new AppError("Mã xác nhận không tồn tại", 400);

    if(user.reset_token !== hashResetCode) throw new AppError("Mã xác nhận không đúng", 400);

    if(user.reset_token_expire < new Date()) throw new AppError("Mã xác nhận đã hết hạn", 400);

    return true;
}

export const resetPasswordService = async(email: string, resetCode: string, newPassword: string, confirmPassword: string) => {
    ...
    if(isSamePassword) throw new AppError("Mật khẩu mới không được trùng mật khẩu cũ", 400);
...
export const refreshTokenService = async(refreshToken: string) => {
    ...
    if(!user || !user.refreshToken) throw new AppError("Refresh token không hợp lệ", 401);

    const hashRefreshToken = hashToken(refreshToken);

    if(user.refreshToken !== hashRefreshToken) throw new AppError("Refresh token không hợp lệ", 401);
...
export const logoutService = async(userId: string, token: string) => {
    ...
    if(!decoded || !decoded.exp) throw new AppError("Token không hợp lệ", 401);
