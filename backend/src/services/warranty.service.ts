import { 
    createWarrantyWithProcess, 
    findDeviceBySerialForWarranty, 
    findMyWarrantyById, 
    findOpenWarrantyByDeviceId, 
    findWarrantyById, 
    getMyWarranties, 
    getWarrantiesWithQuery, 
    updateWarrantyStatusWithProcess 
} from "#models/warranty.model";
import {
    findDeviceByIdForWarranty,
    findWarrantyOrderDetailsByPhone,
} from "#models/warranty.model";
import {
    AddWarrantyProcessPayload,
...
function isWarrantyValid(warrantyEndDate?: Date | null) {
...
function assertWarrantyStatus(
    currentStatus: warranties_status,
    allowedStatuses: warranties_status[],
    message: string,
) {
...
export const lookupWarrantyBySerialService = async(serialNumber: string) => {
...
export const lookupWarrantyByPhoneService = async(customerId: string, phoneNumber: string) => {
...
export const createWarrantyService = async(customerId: string, data: CreateWarrantyPayload) => {
    const device = data.deviceId
        ? await findDeviceByIdForWarranty(data.deviceId.trim())
        : await findDeviceBySerialForWarranty((data.serialNumber || "").trim());

    if(!device) throw new AppError("Không tìm thấy sản phẩm trong hệ thống.", 404);
...
export const getAllWarrantiesService = async(params: WarrantyListQuery) => {
...
export const getWarrantyDetailService = async(warrantyId: string) => {
...
export const getMyWarrantiesService = async(customerId: string, page = 1, limit = 10) => {
...
export const getMyWarrantyDetailService = async(warrantyId: string, customerId: string) => {
...
export const approveWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const rejectWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const markCustomerDropOffWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const schedulePickupWarrantyService = async(employeeId: string, warrantyId: string, data: SchedulePickupPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const markPickedUpWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const receiveWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);

    assertWarrantyStatus(
        warranty.status,
        [warranties_status.APPROVED, warranties_status.CUSTOMER_DROP_OFF, warranties_status.PICKED_UP],
        "Chỉ có thể tiếp nhận máy khi yêu cầu đã được duyệt, khách mang tới cửa hàng hoặc shipper đã lấy máy.",
    );
...
export const addWarrantyProcessService = async(employeeId: string, warrantyId: string, data: AddWarrantyProcessPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const inspectWarrantyService = async(employeeId: string, warrantyId: string, data: InspectWarrantyPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const startRepairWarrantyService = async(employeeId: string, warrantyId: string, data: RepairWarrantyPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);

    assertWarrantyStatus(
        warranty.status,
        [warranties_status.INSPECTING, warranties_status.WAITING_CUSTOMER_CONFIRMATION, warranties_status.BRAND_RETURNED],
        "Chỉ có thể bắt đầu sửa chữa sau khi đã kiểm tra hoặc hãng đã trả máy.",
    );
...
export const sendToBrandWarrantyService = async(employeeId: string, warrantyId: string, data: SendToBrandPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const brandReturnedWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const completeWarrantyService = async(employeeId: string, warrantyId: string, data: RepairWarrantyPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const scheduleReturnWarrantyService = async(employeeId: string, warrantyId: string, data: ScheduleReturnPayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const returnWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
...
export const cancelWarrantyService = async(employeeId: string, warrantyId: string, data: WarrantyNotePayload) => {
    const warranty = await findWarrantyById(warrantyId);
    if(!warranty) throw new AppError("Không tìm thấy phiếu bảo hành.", 404);
